...
export function ParticleSphere({ images }: ParticleSphereProps) {
  ...
  const orbitingImages = useMemo(() => {
    ...
    for (let i = 0; i < IMAGE_COUNT; i++) {
      ...
      // By pointing lookAt towards the center, the negative Z axis points inwards,
      // which means the positive Z axis (the front side of the plane) points outwards.
      ...
      {orbitingImages.map((image, index) => (
        <group key={`image-group-${index}`} position={image.position} rotation={image.rotation}>
          ...
          <mesh scale={[1.04, 1.04, 1]}>
            <planeGeometry args={[IMAGE_SIZE * image.aspectRatio, IMAGE_SIZE]} />
            <meshBasicMaterial 
